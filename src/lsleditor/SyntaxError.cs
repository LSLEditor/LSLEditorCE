...
namespace LSLEditor
{
    public partial class SyntaxError : DockContent
    {
        ...
        {
            OnSyntaxError?.Invoke(this, new SyntaxErrorEventArgs(strPath, guid, isScript, intLine, intChar));
        }
        ...
        private void listView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (this.listView1.SelectedItems.Count == 1)
            {
                ...
                if (int.TryParse(lvi.SubItems[4].Text, out var intLine)
                 && int.TryParse(lvi.SubItems[5].Text, out var intChr))
                {
                    var strPath = lvi.SubItems[7].Text;
                    var guid = new Guid(lvi.SubItems[8].Text);
                    var isScript = Convert.ToBoolean(lvi.SubItems[9].Text);
                    this.DoSyntaxError(strPath, guid, isScript, intLine, intChr);
                }
            }
        }
        ...
        private void listView1_Resize(object sender, EventArgs e)
        {
            ...
            var intWidth = 0;
            for (var intI = 0; intI < this.listView1.Columns.Count; intI++)
            {
                if (intI != 2)
                {
