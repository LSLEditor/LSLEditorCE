    ...
    internal class WebRequestClass
    {
        ...
        public WebRequestClass(WebProxy proxy, SecondLife secondlife, string strUrl, SecondLife.list parameters, string postData, SecondLife.key key)
        {
            try
            {
                ...
                var myWebRequest = WebRequest.Create(strUrl);
                ...
                var RegionCorner = secondlife.llGetRegionCorner();
                var pos = secondlife.llGetPos();
                ...
                var myRequestState = new RequestState
                {
                    ...
                    httpkey = key,
                    postData = Encoding.UTF8.GetBytes(postData)
                };
                ...
                var asyncResult = myWebRequest.Method == "POST" || myWebRequest.Method == "PUT"
                    ? myWebRequest.BeginGetRequestStream(new AsyncCallback(RespCallback), myRequestState)
                    : myWebRequest.BeginGetResponse(new AsyncCallback(RespCallback), myRequestState);
            }
        ...
        private static void RespCallback(IAsyncResult asynchronousResult)
        {
            ...
            try
            {
                ...
                var asynchronousResultRead = responseStream.BeginRead(myRequestState.bufferRead, 0, BUFFER_SIZE, new AsyncCallback(ReadCallBack), myRequestState);
            }
            catch (WebException e)
            {
        ...
        private static void ReadCallBack(IAsyncResult asyncResult)
        {
            ...
            try
            {
                ...
                var responseStream = myRequestState.responseStream;
                ...
                if (read > 0)
                {
                    ...
                    var asynchronousResult = responseStream.BeginRead(myRequestState.bufferRead, 0, BUFFER_SIZE, new AsyncCallback(ReadCallBack), myRequestState);
                }
                else
                {
